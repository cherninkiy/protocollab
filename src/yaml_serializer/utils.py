...
def mark_dirty(node):
    ...
    logger.debug("Marked dirty node %s (old hash: %s, new hash: %s)", type(node).__name__, old_hash, node._yaml_hash)
...
def mark_node(node, filename, parent=None):
    ...
    if isinstance(node, (CommentedMap, CommentedSeq)):
        ...
        logger.debug("Marked node %s with file=%s, parent=%s", type(node).__name__, filename, parent)
        if isinstance(node, CommentedMap):
            for v in node.values():
        ...
        if hasattr(node, '_yaml_include_path') and hasattr(node, '_yaml_parent_file'):
            if getattr(node, '_yaml_file', None) == new_file:
                ...
                try:
                    ...
                    if logger:
                        logger.debug("Updated _yaml_include_path to absolute: %s", node._yaml_include_path)
        if isinstance(node, CommentedMap):
            for v in node.values():
