"""fastjsonschema backend for json_schema_validator.
...
.. warning::
   ...
   Always use the default ``jsonschema`` backend (or ``auto``) when the schema
   ...
   :class:`~json_schema_validator.factory.ValidatorFactory`.

This module is an **optional** backend: if ``fastjsonschema`` is not installed
importing this module will succeed, but constructing
:class:`FastjsonschemaBackend` will raise :class:`ImportError`.
"""
...
def _build_path(path: list[str]) -> str:
    ...
    for segment in path:
        ...
        if segment.startswith("data"):
            # strip the leading "data" prefix and normalize
            inner = segment[len("data"):]
            # "[0]" → append index to last part
